...
describe("InstrumentPanel — bottom split panel", () => {
  ...
  it("3. OVERVIEW shows contract terms; DTE is a bare number (no 'd' suffix)", () => {
    ...
    expect(html).toContain("Strike · VND");
    expect(html).toContain("120,000");
    expect(html).toContain("Maturity");
    expect(html).toContain("Days to maturity");
    expect(html).not.toMatch(/>\s*\d+d\s*</); // never "24d"
  });

  it("3b. Overview prioritises history with a compact unavailable-data note", () => {
    ...
    expect(closed).toContain("Daily history");
    expect(closed).toContain(
      "Trade prints and full order-book depth are not available",
    );
  ...
  it("4. CONFLICTING metadata -> explained in the panel, analytics withheld as em-dash", () => {
    ...
    expect(html).toContain("Quant analytics are withheld until reconciled");
  });
