...
import samplePatches from "./market_patches.json";
...
export class MockMarketDataProvider implements MarketDataProvider {
  ...
  public syncSubscriptions(symbols: string[]): void {
    const nextSet = new Set(symbols.map((s) => s.toUpperCase()));
    this.subscribedSymbols = nextSet;
  }
  ...
  public onMarketSessionChange(listener: (session: { status: string; active: boolean }) => void): () => void {
    listener({ status: "MORNING_SESSION", active: true });
    return () => {};
  }
