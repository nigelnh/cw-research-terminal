...
describe("Quant API contract - frontend consumes backend units correctly", () => {
  ...
  it("explicitly unavailable Greek values clear older values", () => {
    client.handleIncomingMessage({
      type: "analytics_patch",
      symbol: "CHPG2541",
      analytics: { symbol: "CHPG2541", greeks: { delta: 0.3, vega: 20.0 } },
    });
    client.handleIncomingMessage({
      type: "analytics_patch",
      symbol: "CHPG2541",
    ...
    const cw = client.getCoveredWarrant("CHPG2541")!;
    expect(cw.delta).toBeNull();
    expect(cw.vega).toBeNull();
  });
});
