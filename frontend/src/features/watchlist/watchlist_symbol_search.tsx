...
export function matchingSymbols(options: WatchlistSearchOption[], query: string): Set<string> {
  ...
  return new Set((exact ? [exact] : options.filter(option => normalizeSearch(`${option.symbol} ${option.name}`).includes(term))).map(option => option.symbol));
}
...
export function WatchlistSymbolSearch({ options, value, onChange }: {
  ...
  const id = useId();
  const matches = matchingSymbols(options, value);
  const suggestions = value.trim() ? options.filter(option => matches.has(option.symbol)) : options;
  ...
  return <div className="watchlist-search" ref={anchor} onBlur={event => {
    ...
    <div className="watchlist-search-control">
      ...
      <input ref={input} className="mono" role="combobox" aria-label="Search watchlist symbols" placeholder="Search symbol or company"
        value={value} aria-autocomplete="list" aria-expanded={shown} aria-controls={shown ? id : undefined}
        aria-activedescendant={shown ? `${id}-${activeIndex}` : undefined}
        onFocus={() => setOpen(true)}
        onChange={event => { onChange(event.target.value); setActive(0); setOpen(true); }}
        onKeyDown={event => {
          if (event.key === "Escape") { event.preventDefault(); setOpen(false); }
          if (event.key === "ArrowDown" || event.key === "ArrowUp") {
            event.preventDefault(); setOpen(true);
            setActive(current => Math.max(0, Math.min(suggestions.length - 1, current + (event.key === "ArrowDown" ? 1 : -1))));
          }
          if (event.key === "Enter" && shown) { event.preventDefault(); choose(suggestions[activeIndex].symbol); }
        }} />
    ...
    {open && <div className="watchlist-search-results" role="listbox" id={id} aria-label="Watchlist matches">
      {suggestions.map((option, index) => <div key={option.symbol} id={`${id}-${index}`} role="option" aria-selected={index === activeIndex}
        ...
        <span className="watchlist-search-name">{option.name || option.underlying || ""}</span>
