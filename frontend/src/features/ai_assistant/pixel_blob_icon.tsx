...
/** Happy Blob from the user-provided pixel icon kit. */
export function PixelBlobIcon({ size = 32 }: PixelBlobIconProps) {
  const height = Math.round((size * 26) / 32);

  return (
    <span
      className="ai-pixel-blob-motion"
      data-ai-blob-motion="idle-squish"
      style={{
        width: size,
        height,
        display: "block",
    ...
    >
      <svg
        ...
        style={{ width: size, height, display: "block", animation: "none", transform: "none" }}
      >
