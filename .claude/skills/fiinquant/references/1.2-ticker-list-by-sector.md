---
name: Danh sách mã theo ngành
description: Lấy danh sách các mã chứng khoán theo phân ngành ICB (Industry Classification Benchmark).
---

# Danh sách mã theo ngành

**Tên hàm:** client.TickerList(ticker='INDUSTRY_NAME')
...
|---------|-------|---------|---------|---------|
| ticker | tên icb ngành (Ví dụ: BANKS_L2, REAL_ESTATE_L2, STEEL_L4, OIL_AND_GAS_L2, v.v.) hoặc ICB Code (Ví dụ: 8300, 8600, 1757, 0500, v.v.) | str | None | Không |
| tickers | Danh sách tên icb ngành hoặc ICB Code, được dùng nếu muốn lấy danh sách các mã thuộc 2 ngành cùng 1 lúc | List of str | None | Không |
**Lưu ý:**
Chỉ được dùng 1 trong 2 tham số, không được dùng cùng lúc, 1 là gọi danh sách các mã cho 1 ngành thì dùng ticker, nếu muốn gọi cho từ 2
ngành trở lên thì dùng tickers.

Danh sách tên icb ngành để truyền vào hàm TickerList sẽ được mô tả ở mục `references/code_lists.md`
...
```python
list(client.TickerList(ticker='BANKS_L2'))
```
...
# Lấy danh sách mã thuộc ngành ngân hàng L2
tickers = client.TickerList(ticker='BANKS_L2')
print(tickers)
# Output: ['ABB', 'ACB', 'BAB', 'BID', 'BVB', 'CTG', 'EIB', 'HDB', ...]

# Hoặc sử dụng ICB Code
tickers = client.TickerList(ticker='8300')
print(tickers)
```

## Bảng tra cứu tên phân ngành, biến truyền vào hàm và ICB Code
