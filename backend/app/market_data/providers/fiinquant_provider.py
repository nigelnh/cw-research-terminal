    ...
    async def set_subscriptions(self, symbols: List[str]) -> bool:
        ...
        def _start_new_streams():
            try:
                ...
                return trade_stream, bidask_stream, None
            except Exception as e:
                err_msg = str(e)
                logger.error(f"Failed to start FiinQuant streams for symbols {clean_symbols}: {err_msg}")
                return None, None, err_msg

        ts, bs, err = await asyncio.to_thread(_start_new_streams)
        self._trade_stream = ts
        self._bidask_stream = bs
        self._last_error = err

        if ts is None and bs is None and clean_symbols:
            self._upstream_status = "ERROR"
            logger.error(f"FiinQuant stream startup failed: {err}")
            return False
