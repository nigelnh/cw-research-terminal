...
from app.market.state import MarketState, market_state
from app.market.schemas import CanonicalQuote
from app.market.providers.base import MarketDataProvider
from app.market.providers.fiinquant_provider import FiinQuantProvider
...
class SubscriptionManager:
    ...
    def __init__(
        ...
        if provider:
            ...
            self.provider = MockMarketDataProvider(max_symbols=self.max_symbols)
        else:
            self.provider = FiinQuantProvider(max_symbols=self.max_symbols)
    ...
    async def initialize(self) -> bool:
        """Initializes upstream connection on application startup."""
        return await self.provider.connect()

    async def shutdown(self) -> None:
        """Cleans up upstream streams on application shutdown."""
        if self._debounce_task and not self._debounce_task.done():
            self._debounce_task.cancel()
