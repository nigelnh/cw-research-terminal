...
from app.quant.volatility import calculate_historical_volatility
...
class LiveQuantEngine:
    def __init__(self):
        ...
        self._historical_vol_getter = None
    ...
    def set_historical_vol_getter(self, getter_func):
        """Injects function to query underlying historical volatility: (symbol: str) -> Optional[float]"""
    ...
    async def compute_warrant_analytics(
        ...
        # 8. True Theoretical Fair Price (requires independent volatility assumption, e.g. HV)
        # Invariant: Never use IV Mid as theoretical fair value (circular repricing).
        ...
        if self._historical_vol_getter:
            try:
                theo_vol = self._historical_vol_getter(und_sym)
                if theo_vol and theo_vol > 0:
                    theo_vol_src = "HV_66"
