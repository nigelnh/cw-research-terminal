...
class WarrantAnalytics(BaseModel):
    ...
    historical_volatility: Optional[float] = Field(
        default=None, description="Underlying Historical Volatility (30-day/22-bar annualized, decimal)"
    )
    theoretical_price: Optional[float] = Field(
        default=None, description="True Theoretical Fair Price evaluated with independent volatility (e.g. HV_66 / HV_22)"
    )
    theoretical_volatility: Optional[float] = Field(
        default=None, description="Independent volatility used to compute Theoretical Fair Price"
    )
    theoretical_volatility_source: Optional[str] = Field(
        default=None, description="Source of independent theoretical volatility (e.g. HV_66, HV_22, UNAVAILABLE)"
    )
    model_price_at_iv_mid: Optional[float] = Field(
